...
class LinkedBaseModel(BaseModel):
    ...
    def __init__(self, *a, **kw):
        ...
        for name in list(kw): #force copy of keys for inline-delete
            ...
            if not "__iris__" in kw: kw["__iris__"] = {}
            if self.model_fields[name].json_schema_extra and "range" in self.model_fields[name].json_schema_extra: # pydantic v2: model_fields

                arg_is_list = isinstance(kw[name], list)
                
                annotation_is_list = False
                args = self.model_fields[name].annotation.__args__
                if hasattr(args[0],"_name"): is_list = args[0]._name == "List"
                if arg_is_list:
                    kw["__iris__"][name] = []
        ...
        if "__iris__" in self.__dict__:
            if name in self.__dict__["__iris__"]:
                if self.__dict__[name] is None or (
                    ...
                    if not is_list:
                        iris = [iris]
                    node_dict = self.resolver.resolve(
                        ResolveRequest(iris=iris)
                    ).nodes
                    if is_list:
                        node_list = []
                        for iri in iris:
