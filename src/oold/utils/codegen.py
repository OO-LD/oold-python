...
class OOLDJsonSchemaParser(JsonSchemaParser):
    ...
    def set_additional_properties(self, name: str, obj: JsonSchemaObject) -> None:
        ...
        if self.data_model_type == pydantic_v1_model.BaseModel:
            self.extra_template_data[name]["config"] = PydanticV1Config(schema_extra=schema_extras)
        if self.data_model_type == pydantic_v2_model.BaseModel:
            self.extra_template_data[name]["config"] = PydanticV2Config(json_schema_extra=schema_extras)
    ...
    def parse_combined_schema(
        ...
        for index, target_attribute in enumerate(getattr(obj, target_attribute_name, [])):
            if target_attribute.ref:
                ...
                if (
                    hasattr(so, "properties")
                    and hasattr(obj, "properties")
                    and so.properties is not None
                    and obj.properties is not None
                ):
                    for k, v in so.properties.items():
                        if k in obj.properties and obj.properties[k].extras:
                            v.extras = self._deep_merge(v.extras, obj.properties[k].extras)
                        x_of_properties = getattr(target_attribute, "properties", {})
                        if k in x_of_properties and x_of_properties[k].extras:
    ...
    def parse_enum(
        ...
        for r in self.results:
            if r.reference == ref and "options" in obj.extras and "enum_titles" in obj.extras["options"]:
                max_index = min(len(r.fields), len(obj.extras["options"]["enum_titles"]))
                ...
                for i in range(max_index):
                    if "description" not in r.fields[i].extras:
                        r.fields[i].extras["description"] = obj.extras["options"]["enum_titles"][i]
        return schema
...
class OOLDJsonSchemaParserFixedRefs(OOLDJsonSchemaParser):
    ...
    def _get_ref_body_from_remote(self, resolved_ref: str) -> dict[Any, Any]:
        ...
        # resolved_ref:  'C:/Users/Example/Git/OO-LD/oold-python/src/oold/example/src/Bar.json'
        # self.base_path 'C:/Users/Example/Git/OO-LD/oold-python/src/oold/example/src'
        # => full_path:  'C:/Users/Example/Git/OO-LD/oold-python/src/oold/example/src/Bar.json'
        # resolved_ref:  'C:/Users/Example/Git/OO-LD/oold-python/src/oold/example/src/Users/Example/Git/OO-LD/oold-python/src/oold/example/src/bar2/Bar2.json'
        # self.base_path 'C:/Users/Example/Git/OO-LD/oold-python/src/oold/example/src'
        # => full_path:  'C:/Users/Example/Git/OO-LD/oold-python/src/oold/example/src/bar2/Bar2.json'
        ...
        resolved_ref = resolved_ref.split(self.base_path.as_posix().split(":", 1)[-1])[-1].lstrip(
            "/"
        ...
        return self.remote_object_cache.get_or_put(
            str(full_path),
            default_factory=lambda _: self._load_yaml_from_path(full_path, self.encoding),
        )
