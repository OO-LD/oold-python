...
if __name__ == "__main__":
    ...
    q = SelectQuery(
        select_clause=SelectClause(
            distinct=True,
            variables_or_all=[Var(value="s"), Var(value="p"), Var(value="o")],
        ),
        where_clause=WhereClause(
            group_graph_pattern=GroupGraphPattern(
                content=GroupGraphPatternSub(
                    triples_block=TriplesBlock(
                        triples=TriplesSameSubjectPath.from_spo(
                            subject=Var(value="s"),
                            predicate=iri(value="http://example.org/predicate"),
                            object=Var(value="o"),
                        )
                    )
                )
        ...
        solution_modifier=SolutionModifier(
            limit_offset=LimitOffsetClauses(limit_clause=LimitClause(limit=10)),
            having=None,
